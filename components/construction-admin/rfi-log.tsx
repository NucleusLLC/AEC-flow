...
export function RfiLog({ rfis }: { rfis: Rfi[] }) {
  ...
  const rows = useMemo(() => {
    ...
    return rfis.filter((r) => {
      ...
      if (!q) return true;
      return [r.rfiNumber, r.subject, r.projectName, r.assignedTo ?? ""].join(" ").toLowerCase().includes(q);
    });
  }, [rfis, query, status]);
              ...
              {rows.map((r) => (
                <tr key={r.id} className="group transition-colors even:bg-surface-2/60 hover:bg-surface-2">
                  <td className="px-5 py-3">
                    <Link href={`/construction-admin/rfis/${r.id}`} className="block">
                      <span className="font-mono text-[11px] text-faint">{r.rfiNumber}</span>
                      <span className="mt-0.5 block max-w-xs truncate font-medium text-fg group-hover:text-brand">{r.subject}</span>
                    </Link>
                  </td>
