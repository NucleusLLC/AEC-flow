...
export function ChangeOrderRegister({ changeOrders }: { changeOrders: ChangeOrder[] }) {
  ...
  const rows = useMemo(() => {
    ...
    return changeOrders.filter((c) => {
      ...
      if (!q) return true;
      return [c.changeOrderNumber, c.title, c.projectName, c.contractor ?? ""]
        .join(" ")
        .toLowerCase()
        .includes(q);
    });
  }, [changeOrders, query, status]);

  const approvedValue = rows
    .filter((c) => c.status === "APPROVED")
    .reduce((n, c) => n + c.totalCost, 0);
  const currency = changeOrders[0]?.currency ?? "USD";
              ...
              {rows.map((c) => (
                <tr key={c.id} className="group transition-colors even:bg-surface-2/60 hover:bg-surface-2">
                  <td className="px-5 py-3">
                    <Link href={`/construction-admin/change-orders/${c.id}`} className="block">
                  ...
                  <td className="px-3 py-3 text-muted">{c.projectName}</td>
                  <td className="px-3 py-3 text-right font-medium text-fg">{formatCurrency(c.totalCost, c.currency)}</td>
                  <td className="px-3 py-3 text-right text-muted">
                    {c.scheduleImpactDays === 0 ? "—" : `${c.scheduleImpactDays > 0 ? "+" : ""}${c.scheduleImpactDays}d`}
                  </td>
                  <td className="px-5 py-3">
